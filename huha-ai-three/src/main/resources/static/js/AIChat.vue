...
const sendMessage = async () => {
  ...
  try {
    const { reader, title } = await chatAPI.sendMessage(formData, currentChatId.value)
    const decoder = new TextDecoder('utf-8')
    let accumulatedContent = ''
    
    // 如果后端返回了新的标题，更新聊天标题
    if (title) {
      updateChatTitle(currentChatId.value, title)
    }
...
const loadChat = async (chatId) => {
  ...
  try {
    ...
    // 确保消息按时间戳排序
    currentMessages.value = messages.sort((a, b) => a.timestamp - b.timestamp)
  } catch (error) {
...
const loadChatHistory = async () => {
  try {
    ...
    // 确保历史记录按时间戳排序
    chatHistory.value = (history || []).sort((a, b) => b.timestamp - a.timestamp)
